...
export default function EditSalaryStructurePage() {
  ...
  useEffect(() => {
    if (!authLoading && !hasPayrollAccess()) {
      alert('Access denied. Only Admin, Payroll Officers, and HR Officers can edit salary structures.');
      router.push('/dashboard');
      return;
    }
    if (!authLoading && structureId) {
      loadStructure();
    }
  }, [authLoading, structureId]);

  const loadStructure = async () => {
    try {
      setLoading(true);
      const response = await salaryStructureAPI.getById(structureId);
      const data = response.data;
      setStructure(data);

      // Pre-fill form
      setFormData({
        monthlyWage: data.monthlyWage?.toString() || '',
        ...
        professionalTax: data.professionalTax?.toString() || '200',
        workingDaysPerWeek: data.workingDaysPerWeek?.toString() || '5',
        workingHoursPerDay: data.workingHoursPerDay?.toString() || '8',
      });
    } catch (error: any) {
      console.error('Failed to load salary structure:', error);
      alert('Failed to load salary structure: ' + error.message);
      router.push('/dashboard/payroll/salary-structure');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirm = window.confirm(
      `Update salary structure to monthly wage ₹${wage.toLocaleString('en-IN')}?`
    );
    if (!confirm) return;
    ...
    try {
      ...
      await salaryStructureAPI.update(structureId, payload);
      alert('Salary structure updated successfully!');
      router.push('/dashboard/payroll/salary-structure');
    } catch (error: any) {
      alert('Failed to update salary structure: ' + error.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    const confirm = window.confirm(
      `Delete salary structure for ${structure?.employeeName}? This action cannot be undone and may affect future payrun processing.`
    );
    if (!confirm) return;
  ...
  if (!structure) {
    return <div>Salary structure not found</div>;
  }
